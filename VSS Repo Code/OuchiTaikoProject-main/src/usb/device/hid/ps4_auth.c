...
void ps4_auth_init(const char *private_key, size_t private_key_len, const uint8_t serial[PS4_AUTH_SERIAL_LENGTH],
    ...
    // CRITICAL: Always deinit first to clear any stale state from previous mode
    ps4_auth_deinit();
...
void ps4_auth_deinit() {
    // Clear ALL auth state including the initialized flag and callback
    // This prevents stale PS4 auth state from persisting through reboots
    memset(&auth_state, 0, sizeof(auth_state));
    ps4_auth_sign_cb = NULL;
}
