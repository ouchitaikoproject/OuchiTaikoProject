...
I <i>highly</i> recommend purchasing the raw Controller PCB from OSHPARK PCB Fabrications. They are in the US, and all manufacturing is done in the US. They are very reasonably priced, and you can even pay for expedited production/shipping. They sell in sets of 3 only, but that price is still reasonable, and that way you have a spare in case you damage one, or you can split the order with a friend.

Here is a link to the Board that's already uploaded and verified on their site, and you can hit purchase right away. I do not make any money off of this link - they just offer the convenient service of allowing me to upload the board so that people can simply add it to their cart and check out without the hassle of uploading the raw files and hoping everything is correct and verified after the upload, etc.
...
4. Confirm the thresholds are `225 / 190 / 190 / 225`.
...
3. For each pad, do:
   - `5` fast normal hits
   - `1` hard hit
4. If bleed is detected, the controller will raise only the offending pad and ask you to hit the same target pad again.
