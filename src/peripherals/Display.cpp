...
void Display::showTantrumInstructions() {
    m_state = State::TantrumInstructions;
}
...
void Display::showTantrumCalibrating() {
    m_state = State::TantrumCalibrating;
}
...
// ============================================
// OLD BOOT SPLASH - Gravity Drop with Particles
// ============================================
/*
...
void Display::drawBootSplash() {
    ...
    if (elapsed < PHASE1_END) {
        ...
        if ((elapsed / 250) % 2 == 0) {
            ...
            // Stock font at scale 2: base font is 5px wide + 1px spacing = 6px per char
            // At scale 2: 6 * 2 = 12px per char
            // 11 chars = 132px total width
            int text_width = strlen(text) * 6 * 2;  // More accurate calculation
...
void Display::drawIdleScreen() {
    ...
    // Streak counter removed - was just cosmetic novelty feature
    // Space at y=24 now available for larger animations if desired
    ...
    if (button_label) {
        // Draw centered horizontally below header
...
// ============================================================================
// AUTO CALIBRATE SCREENS
// ============================================================================

void Display::drawTantrumInstructionsScreen() {
    // INSTRUCTION SCREEN - Wait for user to press CONFIRM
    
    // Header
    const char* header = "AUTO CALIBRATE";
    int header_x = (128 - strlen(header) * 6) / 2;
    ssd1306_draw_string(&m_display, header_x, 2, 1, header);
    
    ssd1306_draw_line(&m_display, 0, 12, 127, 12);
    
    // Instructions (3 lines explaining what to do)
    const char* line1 = "When calibration";
    int line1_x = (128 - strlen(line1) * 6) / 2;
    ssd1306_draw_string(&m_display, line1_x, 16, 1, line1);
    
    const char* line2 = "starts, hit all 4";
    int line2_x = (128 - strlen(line2) * 6) / 2;
    ssd1306_draw_string(&m_display, line2_x, 24, 1, line2);
    
    const char* line3 = "pads randomly with";
    int line3_x = (128 - strlen(line3) * 6) / 2;
    ssd1306_draw_string(&m_display, line3_x, 32, 1, line3);
    
    const char* line4 = "drum rolls &";
    int line4_x = (128 - strlen(line4) * 6) / 2;
    ssd1306_draw_string(&m_display, line4_x, 40, 1, line4);
    
    const char* line5 = "standard hits!";
    int line5_x = (128 - strlen(line5) * 6) / 2;
    ssd1306_draw_string(&m_display, line5_x, 48, 1, line5);
    
    // Footer - consistent with menu system (B:Back on left, A:OK on right)
    const char* footer = "B:Back  A:Start";
    int footer_x = (128 - strlen(footer) * 6) / 2;
    ssd1306_draw_string(&m_display, footer_x, 56, 1, footer);
}
...
void Display::drawTantrumCountdownScreen() {
    ...
    // Auto-transition to calibrating when countdown expires
    if (tantrum_state.current_mode == Drum::TantrumState::Mode::Calibrating) {
        if (m_state != State::TantrumCalibrating) {
            showTantrumCalibrating();
        }
        return;
    }
...
void Display::drawTantrumCalibratingScreen() {
    if (!m_drum) return;
    const auto& tantrum_state = m_drum->getTantrumState();

    // Auto-transition when phase finishes
    ...
    // HEADER - "HIT DRUMS!" (10 chars × 12px = 120px)
    const char* header = "HIT DRUMS!";
    int header_x = (128 - strlen(header) * 12) / 2;
    ssd1306_draw_string(&m_display, header_x, 0, 2, header);

    // INSTRUCTIONS (3 lines, size 1 font)
    const char* line1 = "Mix drum rolls &";
    int line1_x = (128 - strlen(line1) * 6) / 2;
    ssd1306_draw_string(&m_display, line1_x, 20, 1, line1);

    const char* line2 = " standard hits on";
    int line2_x = (128 - strlen(line2) * 6) / 2;
    ssd1306_draw_string(&m_display, line2_x, 28, 1, line2);

    const char* line3 = "   all 4 pads!";
    int line3_x = (128 - strlen(line3) * 6) / 2;
    ssd1306_draw_string(&m_display, line3_x, 36, 1, line3);

    // PROGRESS BAR (y=48, width 100px)
    const int BAR_X = 14;
    const int BAR_WIDTH = 100;
    uint16_t bar_width = static_cast<uint16_t>(tantrum_state.getProgress() * BAR_WIDTH);
    ssd1306_draw_line(&m_display, BAR_X, 48, BAR_X + bar_width, 48);
    ssd1306_draw_line(&m_display, BAR_X, 49, BAR_X + bar_width, 49);
    ssd1306_draw_line(&m_display, BAR_X, 50, BAR_X + bar_width, 50);

    // TIMER (right side)
    uint32_t seconds = tantrum_state.getSecondsRemaining();
    std::string time_str = std::to_string(seconds) + "s";
    ssd1306_draw_string(&m_display, 116, 48, 1, time_str.c_str());
}
...
void Display::activateRing(uint8_t drum_index) {
    ...
    for (int i = 0; i < MAX_RINGS_PER_DRUM; i++) {
        if (!m_rings[drum_index][i].active) {
            ...
            // Start at radius 0, expand to 17 (larger than outline at 14)
    ...
    for (int drum = 0; drum < 4; drum++) {
        for (int ring = 0; ring < MAX_RINGS_PER_DRUM; ring++) {
            if (m_rings[drum][ring].active) {
                // EXPAND outward (max radius 14 to stay clear of button text at y=23 and footer at y=54)
...
// Draws a BLACK circle outline (clears pixels for ripple effect)
void Display::drawCircleRingBlack(uint8_t x_center, uint8_t y_center, uint8_t radius) {
    ...
    // Lambda to CLEAR pixels (draw black) instead of setting them
    auto clear_pixel_safe = [&](int px, int py) {
        int draw_x = x_center + px;
        int draw_y = y_center + py;
        // Bounds check
        if (draw_x >= 0 && draw_x < 128 && draw_y >= 0 && draw_y < 64) {
            ssd1306_clear_pixel(&m_display, draw_x, draw_y);  // Turn pixel OFF
        }
    };

    while (x >= y) {
        // Draw the 8 octants (in black)
...
void Display::update() {
    ...
    case State::Menu:
        drawMenuScreen();
        break;
    case State::TantrumInstructions:
        drawTantrumInstructionsScreen();
        break;
    case State::TantrumCountdown:
        drawTantrumCountdownScreen();
        break;
    case State::TantrumCalibrating:
        drawTantrumCalibratingScreen();
        break;
