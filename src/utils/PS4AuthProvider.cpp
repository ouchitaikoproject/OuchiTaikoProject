...
PS4AuthProvider::PS4AuthProvider() {
    if (!OuchiTaiko::Config::PS4Auth::config.enabled) {
        return;
    }
    ...
    if (mbedtls_pk_parse_key(&m_pk_context,
                             reinterpret_cast<const unsigned char *>(OuchiTaiko::Config::PS4Auth::config.key_pem.c_str()),
                             OuchiTaiko::Config::PS4Auth::config.key_pem.size() + 1, nullptr, 0, const_rng, nullptr) != 0) {
        return;
    }
