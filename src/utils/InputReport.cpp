...
usb_report_t InputReport::getDebugReport(const InputState &state) {
    ...
    // Stream all four raw values every poll so the web calibration tool
    // can animate live signal bars continuously.
    // Format: "KAL DON_L DON_R KAR\n" (space-separated, 0-4095 each)
    out << drum.ka_left.raw  << " "
        << drum.don_left.raw  << " "
        << drum.don_right.raw << " "
        << drum.ka_right.raw  << "\n";
