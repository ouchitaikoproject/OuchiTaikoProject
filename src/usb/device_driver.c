...
void usbd_driver_init(usb_mode_t mode) {
    ...
    static usb_mode_t previous_mode = USB_MODE_DEBUG;
    static bool first_init = true;
    
    // CRITICAL: Clean up when switching between ANY modes
    if (!first_init && previous_mode != mode) {
        // If we're leaving a PS4 mode, clean up auth state first
        if (previous_mode == USB_MODE_PS4_TATACON || previous_mode == USB_MODE_DUALSHOCK4) {
            ps4_auth_deinit();
            sleep_ms(50);
        }
        
        // ALWAYS disconnect, deinit, and reset when changing modes
        tud_disconnect();
        sleep_ms(250);
        ...
        // Extra cleanup for keyboard modes (they have different endpoint configs)
        if (previous_mode == USB_MODE_KEYBOARD_P1 || previous_mode == USB_MODE_KEYBOARD_P2) {
            sleep_ms(100);  // Extra delay for keyboard cleanup
        }
    }
    ...
    // Initialize TinyUSB with new driver
